...
import { Play, Pause, Download, Sparkles, RefreshCw, Volume2, Video, Scissors, BookOpen, Monitor, FileText, Subtitles } from 'lucide-react';
...
export interface RealImg2ThreeJSScene {
  id: string;
  title: string;
  ...
  cameraMode: 'flat_static' | 'flat_pan' | 'flat_zoom' | 'orbit_360';
}

const CLASSICAL_RED_RIDING_HOOD_SCENES: RealImg2ThreeJSScene[] = [
  {
    id: 'classical-scene-1',
    title: '第一幕：母親的叮嚀與出發',
    characterName: '小紅帽與媽媽',
    dialog: '在寧靜的小鎮裡，小紅帽準備穿過森林去探望生病的外婆。媽媽遞給她一籃糕點與葡萄酒，溫柔叮嚀：『親愛的孩子，路上千萬別離開小徑，也不要和陌生人說話喔！』',
    duration: 6,
    bgImageUrl: '/assets/bg_forest.png',
    charImageUrl: '/assets/red_hood_vector.png',
    displacementScale: 0.2,
    cameraMode: 'flat_static'
  },
  {
    id: 'classical-scene-2',
    title: '第二幕：森林巧遇大灰狼',
    characterName: '大灰狼',
    dialog: '小紅帽提著籃子走在陽光灑落的森林小徑上，狡猾的大灰狼悄悄靠近，露出親切的笑容問：『可愛的小紅帽，你要去哪裡呀？要不要採一些美麗的花朵送給外婆呢？』',
    duration: 6,
    bgImageUrl: '/assets/bg_forest.png',
    charImageUrl: '/assets/wolf.png',
    displacementScale: 0.2,
    cameraMode: 'flat_static'
  },
  {
    id: 'classical-scene-3',
    title: '第三幕：大灰狼先一步闖入外婆家',
    characterName: '外婆',
    dialog: '大灰狼騙小紅帽去採花後，便急忙跑到外婆的小木屋，推開門吞下了可憐的外婆，並戴上外婆的睡帽與眼鏡，偷偷躺在床上假扮成外婆！',
    duration: 6,
    bgImageUrl: '/assets/bg_cabin.png',
    charImageUrl: '/assets/grandma.png',
    displacementScale: 0.2,
    cameraMode: 'flat_static'
  },
  {
    id: 'classical-scene-4',
    title: '第四幕：臥室經典問答「外婆耳朵怎麼這麼大」',
  ...
  {
    id: 'classical-scene-5',
    title: '第五幕：獵人解救大團圓',
    characterName: '勇敢的獵人',
    dialog: '正當危急時刻，路過木屋的勇敢獵人衝進房內制服了大灰狼，成功救出了外婆與小紅帽！大家圍在桌旁品嚐糕點，故事有了平安快樂的結局！',
    duration: 7,
    bgImageUrl: '/assets/bg_cabin.png',
    charImageUrl: '/assets/hunter.png',
    displacementScale: 0.2,
    cameraMode: 'flat_static'
  }
];

const NORTH_WIND_AND_SUN_SCENES: RealImg2ThreeJSScene[] = [
  {
    id: 'wind-sun-scene-1',
    title: '第一幕：北風與太陽的爭論',
    characterName: '北風與太陽',
...
function processImageBackgroundRemoval(
  ...
  if (!ctx) {
    ...
    return { texture: tex, alphaMap: tex };
  }
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    const isWhiteBg = bgR > 220 && bgG > 220 && bgB > 220 && r > 220 && g > 220 && b > 220;
    const isMatchingBg = dist < tolerance;

    if (isMatchingBg || isWhiteBg) {
      data[i + 3] = 0;
      alphaData[i] = 0;
      alphaData[i + 1] = 0;
      alphaData[i + 2] = 0;
  ...
  return { texture, alphaMap };
}

export const RealImg2ThreeJSStudio: React.FC = () => {
  const mountRef = useRef<HTMLDivElement | null>(null);
  const overlayCanvasRef = useRef<HTMLCanvasElement | null>(null);

  const [selectedStory, setSelectedStory] = useState<'wind_and_sun' | 'red_riding_hood'>('wind_and_sun');
  const [scenes, setScenes] = useState<RealImg2ThreeJSScene[]>(NORTH_WIND_AND_SUN_SCENES);
  ...
  const [meshSegments] = useState(64);
  const [autoBgRemove, setAutoBgRemove] = useState(true);
  const [bgTolerance] = useState(45);
  ...
  const [exportFilename, setExportFilename] = useState<string>('fairytale.mp4');
  ...
  const handleStoryChange = (story: 'wind_and_sun' | 'red_riding_hood') => {
    ...
    if (story === 'red_riding_hood') {
      setScenes(CLASSICAL_RED_RIDING_HOOD_SCENES);
    } else {
      setScenes(NORTH_WIND_AND_SUN_SCENES);
    }
  ...
  useEffect(() => {
    ...
    img.src = currentScene.charImageUrl;
    img.onload = () => {
      let charTex: THREE.Texture;
      let alphaMapTex: THREE.Texture | undefined;

      if (autoBgRemove) {
        const processed = processImageBackgroundRemoval(img, bgTolerance);
        charTex = processed.texture;
        alphaMapTex = processed.alphaMap;
      } else {
        charTex = new THREE.CanvasTexture(img);
      }

      const planeWidth = 2.5;
      const planeHeight = 2.5;
      const charGeo = new THREE.PlaneGeometry(planeWidth, planeHeight, meshSegments, meshSegments);
    ...
    const renderLoop = () => {
      ...
      const activeCameraMode = currentScene.cameraMode || cameraMode;

      if (activeCameraMode === 'flat_static') {
        camera.position.set(0, 1.05, 3.6);
        camera.rotation.set(0, 0, 0);
        camera.lookAt(0, 1.05, 0);
      } else if (activeCameraMode === 'flat_pan') {
        camera.position.x = Math.sin(time * 0.4) * 0.2;
        camera.position.y = 1.05;
        camera.position.z = 3.6;
        camera.lookAt(0, 1.05, 0);
      } else if (activeCameraMode === 'flat_zoom') {
  ...
  }, [currentScene, isPlaying, isTestingVoice, depthScale, cameraMode, autoBgRemove, bgTolerance, meshSegments, enableSubtitles]);
  ...
  const handleExportSampleVideo = async () => {
    ...
    // Composite WebGL 3D Canvas + 2D Subtitle Overlay Canvas
    ...
    mediaRecorder.onstop = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const name = `${selectedStory}_story.mp4`;
          setExportedUrl(url);
      ...
      const name = `${selectedStory}_story.webm`;
      setExportedUrl(url);
      setExportFilename(name);
      setIsExporting(false);
      setExportProgress(100);
    ...
    for (let sIdx = 0; sIdx < scenes.length; sIdx++) {
      ...
      for (let f = 0; f < 180; f++) {
        // Draw WebGL frame + Subtitle overlay onto Composite Canvas
          ...
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">支援 AutoSubs 動態字幕燒錄與一鍵導出 .srt 字幕檔！</p>
              </div>
            </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-sm text-slate-200 flex items-center gap-2">
              ...
              【{selectedStory === 'wind_and_sun' ? '北風與太陽' : '正統小紅帽'}】全平面定鏡分鏡列表 ({scenes.length})
            </h3>
          ...
          <div className="space-y-2.5">
            {scenes.map((scene, idx) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <img src={scene.charImageUrl} alt={scene.characterName} className="w-10 h-10 object-contain rounded-lg bg-slate-950 border border-slate-800" />
                  <div>
          ...
          <div className="flex flex-col gap-2 text-xs text-slate-400">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => setAutoBgRemove(!autoBgRemove)}
                  className={`flex items-center gap-1 px-2.5 py-1 rounded-lg font-bold transition ${
                    autoBgRemove
